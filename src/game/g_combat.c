 ...
 */
int
G_LocationDamage(vec3_t point, gentity_t* targ, gentity_t* attacker, int take)
{
	vec3_t bulletPath;
	vec3_t bulletAngle;
	...
	// The upper body never changes height, just distance from the feet
		...
		case LOCATION_FACE:
			if (targ->client->lasthurt_location & LOCATION_FRONT)
				take *= 1.5; // Faceshots REALLY suck
			else
				take *= 0.9;
			break;
		case LOCATION_SHOULDER:
			if (targ->client->lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
				take *= 0.9; // Throat or nape of neck
			else
				take *= 0.8; // Shoulders
			break;
		case LOCATION_CHEST:
			if (targ->client->lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
				take *= 0.8; // Belly or back
			else
				take *= 0.6; // Arms
			break;
